/*
 * jsQR v1.4.0 — pure-JS QR decoder, vendored so File Transfer's receiver
 * works on any browser with getUserMedia + canvas (including iOS Safari and
 * Firefox, which never provide the native BarcodeDetector API this used to
 * depend on). No CDN dependency, no build step — this file is loaded as a
 * plain <script> and exposes a global `jsQR(imageData, width, height, opts)`
 * function.
 *
 * Source: https://github.com/cozmo/jsQR (npm: jsqr@1.4.0)
 * License: Apache-2.0 — see /assets/js/jsQR.LICENSE
 * Minified with terser; behaviour unchanged from the published dist/jsQR.js.
 */
